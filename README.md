...
GPU 機は共用なので、**VRAM を握ったまま放置しない**。ただし普段は何もしなくてよい——
解放は自動化してある:

| 抜け方 | 何が起きるか |
|---|---|
| `/exit` や Ctrl+C で終了 | ランチャが **g24 のモデルを自動アンロード**する(`[OK] g24 のモデルをアンロードした` と出る) |
| **窓を × で強制終了** | ランチャは走れないが、g24 側の `keep_alive`(**10分**)が自動で解放する |
| `fablet -Shutdown` | 上に加えて、Windows 側の常駐(fcc-server と SSH トンネル)も停止する |

つまり × で全部閉じても、最長10分で VRAM は返る。**すぐ確実に返したいときは `fablet -Shutdown`**。

手動で確認・解放したいときは従来どおり:

```bash
./vram.sh             # 今の空きと、誰が使っているか
./cleanup.sh          # ロード中モデルをアンロード
./cleanup.sh --stop   # 長期間使わないなら Ollama サーバごと停止
```

> **fcc-server の窓は出ない。** これは UI を持たない API プロキシ(HTTP で叩くと 401 を返すだけ)で、
> 窓はログが流れるだけだったので隠した。ログは `%TEMP%\fablet-fcc.log` に出る。
